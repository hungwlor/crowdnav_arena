...
MODEL_DIR = '/home/sora/nav2_ws/src/CrowdNav_Prediction_AttnGraph/crowdnav_base/trained_models/GST_predictor_rand'
TEST_MODEL = '41665.pt'
...
def compute_commands(data):
    ...
    with torch.no_grad():
        value, action, _, eval_recurrent_hidden_states = actor_critic.act(
            observation, eval_recurrent_hidden_states, eval_masks, deterministic=True
        )
    obs, rew, done, infos = envs.step(action)
    
    return action[0][0], action[0][1], obs, rew, done, infos
