...
def filter_args(argv, args_to_exclude):
    filtered = []
    skip_next = False
    for arg in argv:
        if skip_next:
            skip_next = False
            continue
        if arg in args_to_exclude:
            # Skip the unwanted argument and its next value (if it has one)
            skip_next = True
            continue
        filtered.append(arg)
    return filtered
...
def get_args():
    # Remove unwanted arguments (e.g., --host and --port)
    unwanted = {"--host", "--port"}
    sys.argv = filter_args(sys.argv, unwanted)
    ...
    # use attn between humans and robots or not (todo: implement this in network models)
